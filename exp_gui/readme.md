...
If you’re new to GUI creation in Factorio, we recommend starting with [Therena's elpful tutorial](https://github.com/ClaudeMetz/UntitledGuiGuide/wiki) to build a solid foundation.
...
- `Elements`: A common local variable name used to store returned element definitions from a GUI module, capitalized to avoid shadowing more generic variable names like `elements`.
...
- `tooltip-`: Prefix used for locale string keys to indicate their intended display location, improving clarity and organization. Others include `caption-` and `error-`
...
By organizing your code into libraries and registering them via add_lib, you also improve modularity; making it easier to reason about each part of your GUI separately and debug issues when they arise.
...
In most cases, the returned definitions are assigned to a local variable named `Elements` (capitalized).
...
This ensures the element is always present and your GUI state is initialized properly without extra boilerplate.
...
The framework includes several pre defined elements that help maintain a consistent appearance and layout across GUI modules.
...
The toolbar also supports player customisation.
...
The framework provides a convenient initializer method, `:element_data{}`, which creates an empty table at `def.data[element]`.
...
This keeps your data management organized and reduces the risk of unintended side effects.
...
All GUI data initializers also accept functions, similar to the `:style` method, enabling you to define dynamic starting states that can change based on the current context.
