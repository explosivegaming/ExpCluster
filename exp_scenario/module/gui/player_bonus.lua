...
Elements.bonus_used._points_limit = {} --- @type table<LuaPlayer, number>

--- Calculate the bonus limit for a player
--- @param player LuaPlayer
--- @return number
function Elements.bonus_used._calculate_points_limit(player)
    local existing = Elements.bonus_used._points_limit[player]
    if not existing then
        local role_diff = Roles.get_role_by_name(config.points.role_name).index - Roles.get_player_highest_role(player).index
        existing = math.floor(config.points.base * (1 + config.points.increase_percentage_per_role_level * role_diff))
        Elements.bonus_used._points_limit[player] = existing
    end
    return existing
end

--- Update the element caption and value with a new bonus cost
--- @param bonus_used LuaGuiElement
--- @param bonus_cost number
--- @return boolean
function Elements.bonus_used.set_bonus_cost(bonus_used, bonus_cost)
    local player = Gui.get_player(bonus_used)
    local limit = Elements.bonus_used._calculate_points_limit(player)
...
--- @param delta number
--- @return boolean
function Elements.bonus_used.apply_delta_cost(bonus_used, delta)
    local player = Gui.get_player(bonus_used)
    local limit = Elements.bonus_used._calculate_points_limit(player)
...
Elements.reset_button = Gui.element("player_bonus/reset_button")
    ...
    :on_click(function(def, player, element)
        ...
        for _, slider in pairs(Elements.bonus_table.data[element_data.bonus_table]) do
            local bonus_data = Elements.bonus_slider.data[slider].bonus_data
            Elements.bonus_slider.set_value(slider, player_data[bonus_data.name] or 0)
        end

        local bonus_cost = Elements.bonus_table.calculate_cost(element_data.bonus_table)
        Elements.bonus_used.set_bonus_cost(element_data.bonus_used, bonus_cost)
    end) --[[ @as any ]]
...
Elements.apply_button = Gui.element("player_bonus/apply_button")
    ...
    :on_click(function(def, player, element)
        ...
        if Elements.bonus_used.set_bonus_cost(element_data.bonus_used, bonus_cost) then
            for _, slider in pairs(Elements.bonus_table.data[element_data.bonus_table]) do
                local bonus_data = Elements.bonus_slider.data[slider].bonus_data
                Elements.container.set_player_bonus(player, bonus_data.name, slider.slider_value)
            end
            Elements.container.apply_bonus(player)
        end
    end) --[[ @as any ]]
...
Elements.bonus_slider = Gui.element("player_bonus/bonus_slider")
    :draw(function(def, parent, bonus_data, elements)
        ...
        local value = player_data[bonus_data.name]
        if not value then
        ...
        local slider_caption = Elements.bonus_slider._slider_caption(bonus_data, value)
        def.data[slider] = {
    ...
    :on_value_changed(function(def, player, element, event)
        ...
        element_data.label.caption = Elements.bonus_slider._slider_caption(bonus_data, value)
        element_data.apply_button.enabled = Elements.bonus_used.apply_delta_cost(element_data.bonus_used, value_change * bonus_data._cost_scale)
        element_data.reset_button.enabled = true
    end) --[[ @as any ]]

--- Get the caption of the slider label
function Elements.bonus_slider._slider_caption(bonus_data, value)
    return bonus_data.is_percentage and format_number(value * 100, false) .. " %" or format_number(value, false)
end

--- Set the value of the slider and set the label caption
function Elements.bonus_slider.set_value(slider, value)
    slider.slider_value = value
    local element_data = Elements.bonus_slider.data[slider]
    element_data.label.caption = Elements.bonus_slider._slider_caption(element_data.bonus_data, value)
    element_data.previous_value = value
end
...
function Elements.bonus_table.calculate_cost(bonus_table)
    ...
    for _, slider in pairs(Elements.bonus_table.data[bonus_table]) do
        local bonus_data = Elements.bonus_slider.data[slider].bonus_data
        cost = cost + slider.slider_value * bonus_data._cost_scale
    end
...
Elements.container = Gui.element("player_bonus/container")
    :draw(function(def, parent)
        ...
        elements.bonus_used = Elements.bonus_used(header, bonus_table)
        ...
        Elements.bonus_used.set_bonus_cost(elements.bonus_used, bonus_cost)
...
--- @param player LuaPlayer
function Elements.container.clear_bonus(player)
...
--- @param player LuaPlayer
function Elements.container.apply_bonus(player)
...
local function recalculate_bonus(event)
    ...
    if event.name == Roles.events.on_role_assigned or event.name == Roles.events.on_role_unassigned then
        ...
        Elements.bonus_used._points_limit[player] = nil

        local within_limit = false
        local bonus_cost = Elements.container.calculate_cost(player)
        for _, element in Elements.bonus_used:online_elements(player) do
            within_limit = Elements.bonus_used.set_bonus_cost(element, bonus_cost)
        end

        if not within_limit or not Roles.player_allowed(player, "gui/bonus") then
            Elements.container.clear_bonus(player)
            return
        end
    end
...
local function apply_personal_battery_recharge(player)
    ...
    local recharge_amount = Elements.container.data[player]["personal_battery_recharge"] * 100000 * config.periodic_bonus_rate / 6
...
local function apply_periodic_bonus_all()
...
return {
    ...
    on_nth_tick = {
        [config.periodic_bonus_rate] = apply_periodic_bonus_all,
    }
}
